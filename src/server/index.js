...
app.get("/api/person/:id/movies", async (req, res) => {
    ...
    try {
      const response = await axios.get(`https://api.themoviedb.org/3/person/${personId}/movie_credits?api_key=${TMDB_API_KEY}`);
  ...
  app.get("/api/movie", async (req, res) => {
    ...
    try {
      const response = await axios.get(`https://api.themoviedb.org/3/search/movie?api_key=${TMDB_API_KEY}&query=${movieTitle}`);
      if (!response.data.results[0]) res.status(404).json({message: "Movie not found"});
      else res.json(response.data.results[0]); // send back the first result
    } catch (error) {
  ...
  app.get("/api/movies", async (req, res) => {
    ...
    for (let i = 1; i <= 50; i++) {
      try {
        const response = await axios.get(`https://api.themoviedb.org/3/discover/movie?api_key=${TMDB_API_KEY}&page=${i}`);
